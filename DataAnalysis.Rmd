...
# knitr
knitr::opts_chunk$set(warning=FALSE, message=FALSE, fig.height=5, fig.width=6, echo = TRUE)
...
* Figure whats the best model(s) to address this
...
* Try Lotka-Volterra competition coefficients. Ideas: 
  ...
  - C better competitor under control temperature
...
BeforeBleaching1<-subset(qPCR.data, Days<200) # Remoce data points after bleaching
...
* there are very few C dominated cores at the begining of teh experiment and a lot of D dominated cores.
* Dividing the cores by initial D proportion ranks:
  ...
  - Keep details of what happen with >80% D  cores (super interesting... small differences in the initial proportion determine how fast the cores become C dominated)
...
# Initial Proportion Model by rank 
  ...
  # Fitted values and plot Change in DProportion ~ days for different Colonies
  
  resdf <- expand.grid(Days=0:200,
                       D.PrpRank_Ini=unique(BeforeBleaching1$D.PrpRank_Ini))
  
  resdf$fit <- predict(D.Prp_Rank, newdata=resdf, type="response")

  plot(NA, xlim=c(0,200), ylim=c(0,1), yaxs="r",       
       xlab="Days under control temperature (26C)",
       ylab="Proportion of clade D")
  reslist <- split(resdf, f=resdf$D.PrpRank_Ini)
  for (i in 1:length(reslist)) {
    with(reslist[[i]], lines(fit ~ Days, lwd=1))
  }
  abline(v=c(90,180), col=("blue"), lty=2) # Add lines for ~3 and 6 months
  abline(h=0.5, col=("red"), lty=3) # Add a line for 50% CD communities
...
D.Prp_Colony<-glm (D.Prp~ Days + Colony + Tre_1 , family = quasibinomial,
    ...
    resdf$fit <- predict(D.Prp_Colony, newdata=resdf, type="response")
    plot(NA, xlim=c(0,200), ylim=c(0,1),        
         xlab="Days under control temperature",
         ylab="Proportion of clade D")
    reslist <- split(resdf, f=resdf$Colony)
    for (i in 1:length(reslist)) {
      with(reslist[[i]], lines(fit ~ Days, lwd=1))
    }
    
    # Why is this plotting linear data also??  
...
# Initial Proportion Model by rank 
  ...
  resdf <- expand.grid(Days=0:690, 
                       D.PrpRank_Ini=unique(NotBleached$D.PrpRank_Ini))
  
  resdf$fit <- predict(D.Prp_Rank_2, newdata=resdf, type="response")
  
  plot(NA, xlim=c(0,690), ylim=c(0,1), yaxs="r",       
       xlab="Days at ambient tempearture (26-29C)",
       ylab="Proportion of clade D")
  reslist <- split(resdf, f=resdf$D.PrpRank_Ini)
  for (i in 1:length(reslist)) {
    with(reslist[[i]], lines(fit ~ Days, lwd=1))
  }
  abline(v=c(180,360,540, 720), col=("blue"), lty=2) # Add lines for ~6, 12, 18 and 24 months
  abline(h=0.5, col=("red"), lty=3) # Add a line for 50% CD communities
  
   
  summary(BeforeBleaching1$D.PrpRank_Ini)
  

```

CI FOR THIS PREDICTIONS???